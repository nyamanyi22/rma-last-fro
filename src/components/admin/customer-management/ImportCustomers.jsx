...
import { customerApi } from "../../../services/api/api";

const ImportCustomers = ({ onImportComplete, onCancel }) => {
    ...
    const handleImport = async () => {
        ...
        try {
            // Read the file and parse it to pass data as JSON to match Sales import style if preferred,
            // or send as FormData if the backend expects it.
            // Based on SalesController.php, it expects 'sales' as an array in the body.
            // So we'll parse and send JSON.
            ...
            Papa.parse(file, {
                ...
                complete: async (results) => {
                    try {
                        const response = await customerApi.importCustomers({
                        ...
                        if (response.data.success) {
                            setImportResult({
                                success: true,
                                message: response.data.message || "Archive synchronization complete",
                                imported: response.data.imported || [],
                                failed: response.data.failed || [],
                                duplicates: response.data.duplicates || [],
                            });

                            if (response.data.failed?.length === 0 && response.data.duplicates?.length === 0) {
                                setTimeout(() => {
                                    onImportComplete();
                                }, 2000);
                            }
                        } else {
                            setErrors([response.data.message || "Batch ingestion failure"]);
                        }
                    } catch (error) {
                        const errorMsg = error.response?.data?.message || error.message || "Batch ingestion failure";
                        setErrors([errorMsg]);
                    } finally {
                        setImporting(false);
                    }
    ...
    const downloadSampleCSV = () => {
        ...
        const sampleRow = [
            "John", "Doe", "john.doe@example.com", "+1234567890", "US", "New York", "123 Main St", "10001"
        ].join(",");
        ...
        link.href = url;
        link.download = "customer_ingestion_blueprint.csv";
        link.click();
    };
                ...
                <Stack alignItems="center" spacing={3} sx={{ py: 4, textAlign: 'center' }}>
                    <Box sx={{ position: 'relative' }}>
                        ...
                        {(importResult.failed?.length > 0 || importResult.duplicates?.length > 0) && (
                            <Chip
                                label={`${importResult.failed.length + importResult.duplicates.length} Issues`}
                                color="warning"
                                size="small"
                    ...
                    <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                            {importResult.imported?.length || 0} member accounts successfully merged.
                        </Typography>
                    </Box>
                            ...
                            <List dense sx={{ maxHeight: 150, overflow: 'auto' }}>
                                ...
                                {importResult.duplicates.map((d, i) => (
                                    <ListItem key={i} sx={{ px: 0 }}>
                                        ...
                                        <ListItemText
                                            primary={`Duplicate: ${d.email}`}
                                            secondary="Identity already coexistence in database"
                                            primaryTypographyProps={{ variant: 'caption', fontWeight: 800 }}
                                            secondaryTypographyProps={{ variant: 'caption' }}
                                        />
                                    </ListItem>
